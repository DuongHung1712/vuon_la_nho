...
const FilterSection = React.memo(({
  ...
  <div className={className}>
    ...
    {/* Search */}
    <div className="mb-5">
      <label className='block text-sm font-medium text-gray-700 mb-2'>{t('collection.search')}</label>
    ...
    {/* Category */}
    <div className="mb-5">
      <label className="block text-sm font-medium text-gray-700 mb-2">{t('collection.category')}</label>
    ...
    {/* Difficulty */}
    <div className="mb-5">
      <label className="block text-sm font-medium text-gray-700 mb-2">{t('collection.difficulty')}</label>
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <button
            key={item.value}
            onClick={() => setDifficulty(difficulty === item.value ? "" : item.value)}
    ...
    {/* Light Requirement */}
    <div className="mb-5">
      <label className="block text-sm font-medium text-gray-700 mb-2">{t('collection.light')}</label>
      <div className="flex flex-wrap gap-2">
        {[
      ...
      <div className="bg-primary-50/50 rounded-lg p-4 border border-primary-100">
        <div className="flex items-center justify-between text-sm font-medium text-primary-700 mb-5">
          <span className="px-3 py-1.5 bg-white rounded-md border border-primary-200 font-semibold">
            {price[0].toLocaleString()}{t('common.currency')}
          </span>
          <span className="text-gray-400">—</span>
          <span className="px-3 py-1.5 bg-white rounded-md border border-primary-200 font-semibold">
            {price[1].toLocaleString()}{t('common.currency')}
          </span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {filterProducts.length} {t('collection.productsFound', { count: filterProducts.length }) || 'sản phẩm'}
            </p>
          </div>
          ...
          {filterProducts.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">
                {t('collection.tryAdjustFilters') || 'Thử điều chỉnh bộ lọc để xem thêm sản phẩm'}
              </p>
              <button
                onClick={resetFilters}
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute inset-y-0 right-0 w-full max-w-sm bg-white shadow-xl overflow-y-auto animate-slide-left">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between z-10">
              <h2 className="text-lg font-semibold text-gray-800">{t('collection.filters')}</h2>
              <button
                onClick={() => setShowMobileFilters(false)}
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 p-4">
              ...
              >
                {t('collection.showResults', { count: filterProducts.length }) || `Xem ${filterProducts.length} sản phẩm`}
              </button>
            </div>
          </div>
        </div>
