...
const Orders = () => {
  ...
  const getStatusConfig = (status) => {
    const configs = {
      "Đã nhận đơn": {
        ...
        label: t("orders.statuses.orderReceived"),
      },
      "Đang đóng gói": {
        color: "bg-amber-50 text-amber-700 border-amber-200",
        icon: <Package className="w-4 h-4" />,
        dotColor: "bg-amber-500",
        label: t("orders.statuses.packing"),
      },
      "Đã gửi đi": {
      ...
      "Đang trên đường": {
        ...
        label: t("orders.statuses.inTransit"),
      },
  ...
  return (
    <div className="py-8 px-4 sm:px-[5vw] md:px-[7vw] lg:px-[9vw] ">
      <SEO noindex={true} title={`${t("orders.title1")} ${t("orders.title2")} - Vườn Lá Nhỏ`} />
      {/* Header */}
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <Title text1={t("orders.title1")} text2={t("orders.title2")} />
            <p className="text-sm text-gray-500 mt-1">
              {orderData.length} {t("orders.itemsOrdered")}
            </p>
          </div>
          ...
          >
            <RefreshCw
              ...
              strokeWidth={2}
            />
            {isFetching ? t("orders.loading") : t("orders.refresh")}
          </button>
        </div>
      </div>
      ...
      {orderData.length > 0 ? (
              ...
              >
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 sm:gap-3">
                    <span
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium border ${statusConfig.color}`}
                    >
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          >
            <Leaf className="w-4 h-4" /> {t("orders.viewProducts")}
          </a>
        </div>
      )}
    </div>
